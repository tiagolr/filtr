...
{
	freqHP = std::clamp((double)hp, 20., std::min(20000., srate*0.49));
    freqLP = std::clamp((double)lp, 20., std::min(20000., srate*0.49));
...
void Splitter::processBlock(
    int slope, const double* left, const double* right,
    double* lowl, double* lowr, double* midl, double* midr, double* hil,
    double* hir, int nsamps)
{
    if (slope == 0)
...
{
    for (int i = 0; i < nsamps; ++i) {
        double l0 = svfL.lowa.process(left[i]);
        l0 = svfL.lowb.process(l0);
        double m0 = svfL.mida.process(-left[i]);
        m0 = svfL.midb.process(m0);
        double h0 = svfL.higha.process(-left[i]);
        ...
        double l1 = svfR.lowa.process(right[i]);
        l1 = svfR.lowb.process(l1);
        double m1 = svfR.mida.process(-right[i]);
        m1 = svfR.midb.process(m1);
        double h1 = svfR.higha.process(-right[i]);
    ...
    int nsamps)
{
    for (int i = 0; i < nsamps; ++i) {
        ...
        double h0 = svfL.higha2.process(left[i]);
        h0 = svfL.highb.process(h0);
        ...
        double h1 = svfR.higha2.process(right[i]);
        h1 = svfR.highb.process(h1);
